...
describe("browser route chat tabs", () => {
  ...
  it("round-trips stats filters from query params", () => {
    expect(
      browserRouteFromPathname(
        "/stats",
        "?page=2&workspaceId=workspace-1&chatId=chat-1&requestId=request-1%2Crequest-2&requestKind=contextCompression&pageSize=50",
      ),
    ).toEqual({
      filters: {
        chatId: "chat-1",
    ...
    expect(
      ...
      "/stats?page=2&workspaceId=workspace-1&requestId=request-1%2Crequest-2&chatId=chat-1&requestKind=contextCompression&pageSize=50",
    );
  });
