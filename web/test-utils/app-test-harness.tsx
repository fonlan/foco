...
export async function mockFetch(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
  ...
  if (path === "/api/workspaces/workspace-1/spec/generate") {
    if (appTestState.workspaceSpecGenerateCompletes) {
      appTestState.workspaceSpecResponse = {
        ...appTestState.workspaceSpecResponse,
        contentMarkdown:
          "# Project Spec\n\n## Purpose\n\nRegenerated by the LLM.",
        revision: appTestState.workspaceSpecResponse.revision + 1,
        latestJob: { ...workspaceSpecQueuedJob, status: "running" },
      };
      return jsonResponse({
        job: { ...workspaceSpecQueuedJob, status: "running" },
      });
    }
    appTestState.workspaceSpecResponse = {
      ...appTestState.workspaceSpecResponse,
