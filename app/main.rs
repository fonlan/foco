...
use crate::http::settings::{
    ManualApiAuditSettingsRequest, ManualApiProxySettingsRequest, ManualGeneralSettingsRequest,
    ManualSystemPromptRequest, ModelMetadataResponse, WorkspaceCommonCommandSummary,
};
...
fn tool_results_affect_plans(tool_results: &[ExecutedToolCall]) -> bool {
    tool_results.iter().any(|tool_result| {
        !tool_result.is_error
            && matches!(
                tool_result.name.as_str(),
                CREATE_PLAN_TOOL | UPDATE_PLAN_TOOL | UPDATE_PLAN_STEP_TOOL | DELETE_PLAN_TOOL
            )
    })
}
