...
use crate::{Client, Error, ModelIden, PreparedRequestObserver, Result, ServiceTarget};
...
impl Client {
	...
	pub async fn exec_chat_stream(
		...
		self.exec_chat_stream_observed(model, chat_req, options, None).await
	}

	/// Streams a chat response and optionally observes the final reqwest request
	/// after provider adaptation and request overrides, before execution.
	pub async fn exec_chat_stream_observed(
		&self,
		model: impl Into<ModelSpec>,
		chat_req: ChatRequest,
		options: Option<&ChatOptions>,
		observer: Option<PreparedRequestObserver>,
	) -> Result<ChatStreamResponse> {
		let options_set = ChatOptionsSet::default()
			.with_chat_options(options)
			.with_client_options(self.config().chat_options());
		...
		let reqwest_builder = if let Some(observer) = observer {
			let request = reqwest_builder.build().map_err(|source| Error::WebModelCall {
				model_iden: model.clone(),
				webc_error: crate::webc::Error::Reqwest(source),
			})?;
			observer(&request);
			self.web_client().request_builder_from_request(request)
		} else {
			reqwest_builder
		};
