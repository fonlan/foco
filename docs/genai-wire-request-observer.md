...
- `OpenAIRespStreamDiagnostic` is the bounded failure-only diagnostic boundary shared by that decoder. It records `provider_error_event`, `response_failed`, `invalid_json`, `transport_error`, or `unexpected_eof`; the source transport; the prior accepted event and sequence; extracted OpenAI `code` / `type` / `message` / `param`; and a credential-redacted payload snapshot capped at 16 KiB. Snapshots always include original byte count, SHA-256, and truncation state. Successful deltas are never retained. Existing stream APIs are unchanged; capture-aware callers use `exec_chat_stream_observed_with_response_and_diagnostics`, while host WebSocket transports read `OpenAIRespEventDecoder::diagnostics` and report their own socket errors with `record_transport_error`.
