...
- `name`: UI label.
- `hostAlias`: SSH hostname, IP, or OpenSSH config Host alias (JSON field name retained for compatibility; UI labels this “SSH hostname / IP”).
- `user`, `port`, `identityFile`: optional connection fields. Profile values override matching `~/.ssh/config` entries.
- `authMethod`: `key` (default) or `password`. Missing field on old configs deserializes as `key`.
- `password`: optional sensitive login password for `authMethod=password`. Persisted only in local global config; never returned by summary APIs, logs, sidecar bundles, or remote SQLite. Summaries expose `passwordConfigured` only.
- `defaultRemoteRoot`: suggested base directory for new remote workspaces (may be `~`; expanded to an absolute path before workspace create).
...
Never persist private key contents, provider API keys, or provider secrets in this profile. Passwords are machine-local global config secrets (same class as provider keys): never surface in summaries, logs, error details, sidecar bundles, or remote DBs.
...
- `local main`: the user's Foco backend process, owner of global config, provider secrets, pure-Rust SSH client (`russh`), local HTTP API, and UI broker endpoints.
...
| SSH test | `RemoteServerProfile` | local main | Pure-Rust `russh` client (`app/ssh_client`): password or public-key/agent auth, OpenSSH-compatible `known_hosts`, no system `ssh`/`scp`/`sftp`. |
...
- Pure-Rust SSH (`russh`/`russh-config` in `app/ssh_client`): password and public-key/SSH-agent auth; OpenSSH-compatible `known_hosts` (unknown keys require UI fingerprint confirmation; changed keys hard-fail). Passwords may be stored in local global config only; never in summaries, logs, sidecar bundles, or remote SQLite. Encrypted private keys are not unlocked by the login password (use Agent or an unencrypted key).
- Supported OpenSSH config fields (profile wins): HostName, User, Port, IdentityFile, UserKnownHostsFile, StrictHostKeyChecking. `ProxyCommand` and `ProxyJump` are hard errors (no silent direct connect). Nested jump is not implemented.
- Transport: remote command exec, stdin sidecar upload, long-lived exec channel for bootstrap, keepalive, and `direct-tcpip` local forward to remote loopback sidecar port. No system `ssh`/`scp`/`sftp`/`askpass`.
