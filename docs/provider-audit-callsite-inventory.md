...
| 路径 | 调用/写入 | 分类 | 当前状态 |
| --- | --- | --- | --- |
| `app/main.rs` agent turn | `stream_chat_with_capture_observer`、running/outcome audit | 主聊天、多轮工具调用、provider retry | 每个 attempt 先插入空详情 running 行，observers 即时捕获 HTTP `provider_request_v1` 或 WebSocket `provider_websocket_request_v1` 与真实 response/handshake head，Complete/失败/取消写 `provider_final_response_v1`（未捕获则为 NULL）。detail 开启且 OpenAI Responses 流失败时，失败 envelope 可带一个有界、脱敏的 `streamDiagnostic`；每个 retry attempt 保留自己的 envelope，后续 attempt 不覆盖先前失败详情。不再写 run-level `{text,reasoning,...}` / `{error}` / `{cancelled}` 到 `response_body_json`。 |
...
- `streamDiagnostic` 是失败 envelope 的可选 v1 字段，不需要 migration。仅保存单个 16 KiB 上游失败快照的脱敏摘要（不保存完整 SSE/WebSocket 历史）；持久化 envelope 再受 32 KiB 上限保护。payload 内的凭据字段经 decoder 与 Foco redactor 处理；来源 payload 缺失时才使用 fallback 文案。成功流和可忽略未知事件没有诊断字段。
- retention 按既有 `request_detail_retention_days` 清空整列，因此也会清掉 `streamDiagnostic`；Store 白名单只校验 `format=provider_final_response_v1` / `version=1`，会保留这个历史兼容的 optional 字段。SSH sidecar 明细列恒为 NULL；只有主进程 profile audit mirror 可保存并向详情 API 提供诊断。
...
- `providers/lib.rs::tests::openai_resp_detail_stream_exposes_failed_frame_diagnostic` 与 `websocket_failed_provider_event_preserves_decoder_diagnostic_and_handshake_head`：真实 SSE / WebSocket 失败帧保留 provider `code` / `message`、redacted payload 和真实 HTTP 200 / WebSocket 101 head；不把已观察到的 head 反写进重试分类。
...
- `app/tests/mod.rs::main_chat_responses_failed_stream_persists_diagnostic_and_http_head`：mock SSE `response.failed` → SQLite → 详情 JSON，HTTP 200、trace header 与 `streamDiagnostic` 可并存，失败帧凭据不泄露。
- `app/tests/mod.rs::main_chat_retry_attempts_keep_independent_failed_stream_diagnostics`：两个可重试失败 attempt 分别持久化独立的 failed envelope，最终失败不覆盖第一个 attempt 的 provider message 或 stream diagnostic。
...
- `app/remote_workspace.rs::remote_ssh_sidecar_failed_responses_stream_persists_diagnostic_only_in_main_audit_mirror`：真实远程 sidecar 聊天遇到 OpenAI Responses `response.failed` 时，实际 broker request id 关联的主进程 mirror 和详情 API 保留 `streamDiagnostic`、HTTP 200 与 trace header；sidecar 同一行的 request/response detail 仍均为 NULL。
...
- `store/tests/workspace_database.rs::rejects_non_v1_audit_details_and_prunes_legacy_during_explicit_maintenance`：合法 `provider_final_response_v1` 的 optional `streamDiagnostic` 通过白名单维护而保留；`prunes_llm_request_details_without_deleting_statistics` 随后把带该字段的过期失败 envelope 整列清空，证明诊断不会绕过既有 retention。
...
- 流诊断 UI 仅从失败 `provider_final_response_v1.streamDiagnostic` 读取；它显示结构化 kind、transport、事件、provider error、payload 大小/hash/截断状态以及受限 payload excerpt，并允许复制该诊断 JSON。历史记录缺字段、详情关闭、retention 清理或 sidecar 行均保持 unavailable，不从普通错误文案或 run id 伪造诊断。
